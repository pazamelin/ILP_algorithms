...
namespace boost
{
                                     ...
                                     BinaryPredicate compare, BellmanFordVisitor v)
    {
        ...
        for (Size k = 0; k < N; ++k)
        {
            ...
            boost::tie(i, end) = edges(g);
            int edges = num_edges(g);

            tbb::parallel_for(tbb::blocked_range<int>{0, edges},
                [&](const tbb::blocked_range<int>& r)
                {
                    for (int offset = r.begin(); offset < r.end(); ++offset)
                    {
                        auto edge_index = i + offset;
                        v.examine_edge(*edge_index, g);
                        if (relax(*edge_index, g, weight, pred, distance, combine, compare))
                        {
                            at_least_one_edge_relaxed = true;
                            v.edge_relaxed(*edge_index, g);
                        }
                        else
                            v.edge_not_relaxed(*edge_index, g);
                    }
                }
            );
